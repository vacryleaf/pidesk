...
- **阶段**:E 详设(pi-protocol、m1-design 均过环节一;待产出 m1-tasks)。
- **已完成**:pi-protocol v1.0(DR-001/DR-002);m1-design v1.0 环节一通过(2026-09-10 裁决:M1 单连接;dev 数据目录 `<仓库根>/.pidesk-dev`,`PIDESK_DATA_DIR` 可覆盖)。
- **待用户裁决**:m1-tasks.md 任务卡(产出后提请确认,环节一收口)。
- **下一步**:主线程编制 `m1-tasks.md`(按 16k 预算拆卡)→ 用户确认任务卡 → **M1 开工**。
