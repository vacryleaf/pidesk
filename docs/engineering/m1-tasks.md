...
| 卡 | 名称 | 依赖 | 模型 | 核心产出 |
|----|------|------|------|----------|
| T1 | workspace 根 + shared 类型字典 | — | 16k | 根配置;`@pidesk/shared`(协议重导出/IPC 契约/脱敏 + 单测) |
...
| T5 | 进程状态机 | T4 | 16k | pi-host `pi-process.ts` + 单测 |
| T6 | 隔离 env 装配与版本预检 | T5 | 16k | pi-host `env.ts`、`version.ts` + 单测 |
| T7 | 进程池 + L2 真机集成 | T6 | 32k | pi-host `pi-pool.ts` + L2 vitest(spawn 真 pi) |
| T8 | IPC 装配与 preload 桥 | T7 | 16k | main 进程通道注册;preload 白名单桥 |
| T9 | 对话流组件 | T8 | 32k | ui `MessageList`、`Composer` |
| T10 | 会话面板全链路 | T9 | 16k | SessionTabs、状态接线、审批条、toast |
| T11 | 模型菜单与单连接设置 | T10 | 16k | ModelMenu、SettingsDialog → models/auth.json |
| T12 | L4 专项脚本 | T11 | 16k | `scripts/isolation-check.sh`、`scripts/regression-pi.sh` |
| T13 | CI 与 Windows 产物接线 | T12 | 16k | fetch-pi 脚本、electron-builder、GitHub Actions |

出口:T1~T13 全 merged → 主线程跑 L4 + 出口标准核验(m1-design §9)→ 产出 `docs/acceptance/m1-acceptance.md` 验收包 → 提请环节二用户验收(Windows 真机)。

---

## T1 workspace 根 + shared 类型字典
...
- **验收命令**:`pnpm install && pnpm -r typecheck && pnpm -r test`
...
- **验收命令**:`pnpm --filter @pidesk/pi-host test && pnpm -r typecheck`
...
- **验收命令**:`pnpm --filter @pidesk/pi-host test`(L1+L2 全绿)
...
- **目标**:renderer 状态机接 IPC,端到端可对话。
- **产出**:`packages/ui/src/state/session-store.ts`(context+reducer,不引状态库)、`session-view.tsx`、`session-tabs.tsx`、`approval-bar.tsx`、`toast.tsx`;desktop renderer 接线替换占位页。
- **要点(内联源:m1-design §5/§6;ui-prototype §5.1)**:
  - 订阅 `pidesk:event`/`pidesk:process` 按 sessionId 分发 reducer;空态 = 居中引导文案(§5.1,不放假插画);会话标签新建/切换/关闭(关=进程回收)。
  - 审批条:`extension_ui_request confirm` 固着输入框上方(批准/拒绝+风险摘要),不弹窗;notify→toast 3s 自散。
  - 中止/收态:`agent_settled`→Composer 复位;crashed→面板错误态 + 重试按钮。
- **验收命令**:`pnpm --filter @pidesk/ui test && pnpm --filter @pidesk/desktop build && pnpm -r typecheck`
...
- **产出**:`scripts/isolation-check.sh`、`scripts/regression-pi.sh`(可执行,`set -euo pipefail`)。
