 ...
 * assistant = 左对齐 prose,无卡片/气泡;user = 右对齐紧凑中性气泡(bg-2,radius 16px,≤80% 宽)。
...
export function MessageItem({
  ...
  if (role === "user") {
    return (
      <article className="flex justify-end text-left">
        <div className="flex max-w-[80%] flex-col items-end">
          {header}
          <div className="rounded-[16px] bg-[var(--bg-3)] px-4 py-1.5 text-[14px] leading-5 text-[var(--text-0)]">
            {children}
          </div>
        </div>
      </article>
